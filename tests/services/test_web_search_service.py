...
def test_search_summary_returns_standard_envelope(monkeypatch) -> None:
    def fake_search_web_with_playwright(query: str, timeout_seconds: int = 30) -> dict:
    ...
    monkeypatch.setattr(
        "metainflow_studio_cli.services.web_search.service.search_web_with_playwright",
        fake_search_web_with_playwright,
    )
    monkeypatch.setattr(
...
def test_search_summary_does_not_load_model_settings_for_empty_results(monkeypatch) -> None:
    def fake_search_web_with_playwright(query: str, timeout_seconds: int = 30) -> dict:
        return {"provider": "baidu-playwright", "results": []}

    def fail_from_env():
        raise AssertionError("settings should not be loaded for empty results")

    monkeypatch.setattr(
        "metainflow_studio_cli.services.web_search.service.search_web_with_playwright",
        fake_search_web_with_playwright,
    )
    monkeypatch.setattr("metainflow_studio_cli.services.web_search.service.Settings.from_env", fail_from_env)
