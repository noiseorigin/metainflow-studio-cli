...
def test_search_web_falls_back_to_playwright_when_provider_fails(monkeypatch) -> None:
    calls: list[str] = []
    ...
    assert calls == ["provider", "playwright"]
...
def test_search_web_uses_provider_only_when_backend_forced(monkeypatch) -> None:
    ...
    result = search_web("query", _settings(backend="zhipu-web-search"))
